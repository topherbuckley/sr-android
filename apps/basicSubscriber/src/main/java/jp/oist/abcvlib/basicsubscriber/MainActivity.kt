...
class MainActivity : AbcvlibActivity(), SerialReadyListener, BatteryDataSubscriber,
    ...
    override fun onObjectsDetected(
        ...
        try {
            ...
            val category = results[0].categories[0] //todo not sure if there will ever be more than one category (multiple detections). If so are they ordered by highest score?
            val label = category.label
            val score = String.format(Locale.US, "%.2f", category.score)
            val time = String.format(Locale.US, "%d", inferenceTime)
